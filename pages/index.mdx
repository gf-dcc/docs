...
Colloquially, an atlas is a collection of maps illustrating a geographic region. 
In the context of the Gray Foundation consortium, the BRCA Atlas is a superatlas composed of smaller atlases that each team contributes to.

In BRCA-associated cancer, no one team can provide the entirety of the big picture. Thus, metaphorically, each team is a discovery team that:
...
- May initially focus on a region in which they specialize, but should eventually share data and work together without boundaries to create more collaborative atlases.
